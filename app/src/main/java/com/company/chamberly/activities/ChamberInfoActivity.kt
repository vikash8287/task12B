...
class ChamberInfoActivity : ComponentActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_chamber_info)

        // Retrieve the map from the intent
        val chamberMetadata = intent.getSerializableExtra("chamberInfo") as ActiveChatInfoModel

        val textView: TextView = findViewById(R.id.chamberName)
        textView.text = chamberMetadata.groupChatName

        val backButton1: ImageButton = findViewById(R.id.backButton)
        backButton1.setOnClickListener {
            finish()
        }

        val backButton2: ImageButton = findViewById(R.id.back_to_chamber)
        backButton2.setOnClickListener {
            finish()
        }

        val listView: ListView = findViewById(R.id.chamberMembers)

        // Sample data for the ListView
        val data = chamberMetadata.memberInfoList

        // Setting the custom adapter to the ListView
        val adapter = CustomAdapter(this, R.layout.item_chamber_member, data)
